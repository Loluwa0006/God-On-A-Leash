...
public class PlayerSwingState : PlayerAirState
{
    ...
    {
        if (GrappleUtilities.AimingAtGrappable(Player, Player.RodManager.GrappleMask)
            && Player.PlayerInput.BufferRegistry[InputManager.BufferableInputs.Swing].Buffered && 
            Player.RodManager.RodLength <= 0.01f)
        {
            return true;
        }
        return false;
